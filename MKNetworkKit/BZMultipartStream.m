...
- (NSInteger)read:(uint8_t *)buffer maxLength:(NSUInteger)maxlen {
    ...
    NSLog(@"read %d (from %lu) %lu/%lu diff: %lu", read, index, sent, length, length - sent);
...
- (BOOL) setCFClientFlags:(CFOptionFlags)flgs callback:(CFReadStreamClientCallBack)cb context:(CFStreamClientContext*)ctx { return YES; }
...
- (NSString*) description {
    return [[NSString alloc] initWithFormat:@"%@ %d/%d bytes", [super description], sent, length];
}
